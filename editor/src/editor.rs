...
use druid::{Affine, AppLauncher, Color, Data, LocalizedString, Point, Rect, WindowDesc};

pub fn start() {
    let window = WindowDesc::new(EditorWidget::new).title("Map Editor");
...
impl Widget<Editor> for EditorWidget {
    fn event(&mut self, _ctx: &mut EventCtx, _event: &Event, _data: &mut Editor, _env: &Env) {}
    ...
    fn update(&mut self, _ctx: &mut UpdateCtx, _old_data: &Editor, _data: &Editor, _env: &Env) {}
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, data: &Editor, _env: &Env) {
        ...
        let layout = ctx
            .text()
            .new_text_layout(&font, "hello", std::f64::INFINITY)
            .build()
            .unwrap();
